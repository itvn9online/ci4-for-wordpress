...
(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		init: function () {
			...
			list.placeEl = jQuery('<div class="' + list.options.placeClass + '"/>');
		...
		dragStart: function (e) {
			...
			for (i = 0; i < items.length; i++) {
				depth = jQuery(items[i]).parents(this.options.listNodeName).length;
		...
		dragMove: function (e) {
			...
			if (!mouse.dirAx || isNewRoot || isEmpty) {
				...
				if (isEmpty) {
					list = jQuery(document.createElement(opt.listNodeName)).addClass(
						opt.listClass
					);
					list.append(this.placeEl);
