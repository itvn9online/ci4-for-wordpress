...
class Casso
{
    ...
    protected static function pathTestLog()
    {
        ...
        if (!is_file($f)) {
            file_put_contents($f, date('r') . '|' . __CLASS__ . ':' . __LINE__ . "\n", LOCK_EX);
    ...
    public static function testInput($arr)
    {
        ...
        //
        /*
        file_put_contents($f, 'description: ' . $arr->description . "\n", FILE_APPEND);
        file_put_contents($f, 'amount: ' . $arr->amount . "\n", FILE_APPEND);
        file_put_contents($f, 'subAccId: ' . $arr->subAccId . "\n", FILE_APPEND);
        file_put_contents($f, 'bank_sub_acc_id: ' . $arr->bank_sub_acc_id . "\n", FILE_APPEND);
        file_put_contents($f, 'id: ' . $arr->id . "\n", FILE_APPEND);
        file_put_contents($f, 'tid: ' . $arr->tid . "\n", FILE_APPEND);
        */
        /*
        foreach ($arr as $k => $v) {
            file_put_contents($f, $k . ': ' . $v . "\n", FILE_APPEND);
        }
        */
    ...
    public static function phpInput($debug_enable = false, $autobank_token = null)
    {
        ...
        try {
            ...
            file_put_contents($file_log, date('r') . '|' . __CLASS__ . ':' . __LINE__ . "\n", LOCK_EX);
            file_put_contents($file_log, $_SERVER['REQUEST_URI'] . '|' . __CLASS__ . ':' . __LINE__ . "\n", FILE_APPEND);
            //file_put_contents($file_log, __CLASS__ . ':' . __LINE__ . "\n", FILE_APPEND);
            ...
            if (empty($data_string)) {
                ...
                if (1 > 2 && isset($_POST['gfj45fgjfjjg34dfhdfhdhsds34fjfg'])) {
                    ...
                    file_put_contents($file_log, 'empty data string|' . __CLASS__ . ':' . __LINE__ . "\n", FILE_APPEND);
                    return null;
                }
                // TEST data
            ...
            file_put_contents($file_log, $data_string . "\n", FILE_APPEND);
            file_put_contents(self::pathLog(), $data_string . "\n", FILE_APPEND);

            // kiểm tra token trong header (nếu có)
            if (function_exists('getallheaders')) {
                $headers = getallheaders();
                file_put_contents($file_log, json_encode($headers) . "\n", FILE_APPEND);
                file_put_contents($file_log, 'autobank_token: ' . $autobank_token . "\n", FILE_APPEND);

                // 
                if (!empty($autobank_token)) {
                    if (!isset($headers['Secure-Token']) || $headers['Secure-Token'] != $autobank_token) {
                        file_put_contents($file_log, 'Secure-Token mismatch!' . "\n", FILE_APPEND);
                    }
                }
            }
                ...
                foreach ($data->data as $k => $v) {
                    ...
                    if (preg_match($pattern, strtolower($v->description), $matches)) {
                        ...
                        if (is_numeric($order_id) && !empty($order_id)) {
                            ...
                            file_put_contents($file_log, 'count order_id|' . __CLASS__ . ':' . __LINE__ . "\n", FILE_APPEND);
                            $data->data[$k] = null;
                        }
                    }
                }
            } else {
                file_put_contents($file_log, 'data.data not isset|' . __CLASS__ . ':' . __LINE__ . "\n", FILE_APPEND);
                $data = [];
            }
            $result['data'] = $data;
            //$result = $data;
