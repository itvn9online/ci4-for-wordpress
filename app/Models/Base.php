...
class Base extends Csdl
{
    ...
    public function get_add_css($f, $ops = [], $attr = [])
    {
        ...
        return '<link ' . $rel . ' href="' . $ops['cdn'] . $f . '?ver=' . filemtime(PUBLIC_PUBLIC_PATH . $f) . '"' . implode(' ', $attr) . ' />';
    }
    ...
    public function get_add_js($f, $ops = [], $attr = [])
    {
        ...
        //
        if (isset($ops['get_content'])) {
            return '<script {csp-script-nonce}>' . file_get_contents($f, 1) . '</script>';
        }
        ...
        //print_r( $attr );
        return '<script {csp-script-nonce} src="' . $ops['cdn'] . $f . '?ver=' . filemtime(PUBLIC_PUBLIC_PATH . $f) . '" ' . implode(' ', $attr) . '></script>';
    }
    // thêm 1 file
