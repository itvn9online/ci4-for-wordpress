...
class Post extends EB_Model {
    ...
    function insert_post( $data, $tbl = 'wp_posts' ) {
        // các dữ liệu mặc định
        $default_data = [
            'post_author' => $this->session->get( 'admin' )[ 'userID' ],
        ...
        // insert post
        //print_r( $data );
        $result_id = $this->base_model->insert( $tbl, $data, true );
    ...
    function select_post( $post_id, $where = [], $tbl = 'wp_posts' ) {
        if ( $post_id > 0 ) {
            $where[ 'ID' ] = $post_id;
        }
        if ( empty( $where ) ) {
        ...
        $data = $this->base_model->select( '*', 'wp_posts', $where, array(
    ...
    function select_list_post( $post_type, $post_cat = [], $limit = 1, $order_by = [], $ops = [] ) {
        ...
        */
        ...
        if ( isset( $post_cat[ 'term_id' ] ) && $post_cat[ 'term_id' ] > 0 ) {
            $where[ '(wp_term_taxonomy.term_id = ' . $post_cat[ 'term_id' ] . ' OR wp_term_taxonomy.parent = ' . $post_cat[ 'term_id' ] . ')' ] = NULL;
        }
        // tìm theo slug truyền vào
            ...
            $where[ '(wp_term_taxonomy.term_id = ' . $get_term_id[ 'term_id' ] . ' OR wp_term_taxonomy.parent = ' . $get_term_id[ 'term_id' ] . ')' ] = NULL;
        }
        ...
        if ( isset( $ops[ 'count_record' ] ) ) {
            ...
            if ( isset( $ops[ 'select' ] ) ) {
                ...
                //print_r( $ops[ 'select' ] );
                $ops[ 'select' ] = implode( ',wp_posts.', $ops[ 'select' ] );
                //$ops[ 'select' ] = 'wp_posts.' . $ops[ 'select' ];
                //echo $ops[ 'select' ] . '<br>' . "\n";
            } else {
                $ops[ 'select' ] = '*';
            }
            ...
            $data = $this->base_model->select( 'wp_posts.' . $ops[ 'select' ], 'wp_posts', $where, [
                'order_by' => $order_by,
                'join' => [
                    'wp_term_relationships' => 'wp_term_relationships.object_id = wp_posts.ID',
                    'wp_term_taxonomy' => 'wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id',
    ...
    function update_post( $post_id, $data, $where = [], $tbl = 'wp_posts' ) {
    ...
    // thêm post meta
    function insert_meta_post( $meta_data, $post_id, $tbl = 'wp_postmeta' ) {
        //print_r( $meta_data );
    ...
    function get_meta_post( $post_id, $key = '', $tbl = 'wp_postmeta' ) {
        // lấy theo key cụ thể
        if ( $key != '' ) {
    ...
    function arr_meta_post( $post_id, $tbl = 'wp_postmeta' ) {
        $data = $this->get_meta_post( $post_id, '', $tbl );
        //print_r( $data );
    ...
    function quick_add_menu() {
        ...
        foreach ( $allow_post_type as $allow ) {
            ...
            $page_list = $this->base_model->select( '*', 'wp_posts', $where, $filter );
            //print_r( $page_list );
