...
```
$get_data = $this->post_model->get_blogs_by( $term_data, [
	'limit' => 10,
	'offset' => 0,
	'order_by' => [
		'ID DESC',
...
```
$get_data = $this->post_model->get_blogs_by( 2, 10 );
```

##### Lấy `10` bài viết mới nhất của nhóm có slug là `tin-tuc`:
```
$get_data = $this->post_model->get_blogs_by( 'tin-tuc', 10 );
```

#### Function đếm tổng số Bài viết có trong danh mục Blog/ Tin tức:
```
$count_data = $this->post_model->count_blogs_by( $term_data );
```
...
```
$get_data = $this->post_model->count_blogs_by( 2 );
```

##### Tính tổng số bài viết của nhóm có slug là `tin-tuc`:
```
$get_data = $this->post_model->count_blogs_by( 'tin-tuc' );
```
